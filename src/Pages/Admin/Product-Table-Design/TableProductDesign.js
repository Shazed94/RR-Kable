...
const TableProductDesign = () => {
  ...
  const renderAllProducts = async () => {
    await axios
      .get(`${BACKEND_BASE_URL}/api/admin/products/categories/table-designs`)
      .then((res) => {
        setAllProducts(res.data.all_table_designs);
      });
  };
      ...
      <div className="content-wrapper">
        <div className="breadcrumb d-flex justify-content-between">
          ...
          <div className="breadcrumb-item p-0">
            <Link to="#">All Product Design</Link>
          </div>
        </div>
              ...
              <Table responsive>
                ...
                <tbody>
                  {allProducts.map((data, index) => (
                    <tr>
                      <td>{index + 1}</td>
                      <td>{data.design_label}</td>
                      <td>{data.product_category.category_name}</td>
                      {/* <td>{data.cat}</td> */}
...
export default TableProductDesign;
