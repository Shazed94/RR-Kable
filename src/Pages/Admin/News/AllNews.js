...
const AllNews = () => {
  ...
  const renderAllNews = async () => {
    try {
      await axios.get(`${BACKEND_BASE_URL}/api/admin/news`).then((res) => {
  ...
  const showSingleSliderData = (modalValue, id) => {
    axios.get(`${BACKEND_BASE_URL}/api/admin/news/view/${id}`).then((res) => {
      setModalData(true);
  ...
  const deleteData = async (id) => {
    ...
    if (isConfirm) {
      axios
        .delete(`${BACKEND_BASE_URL}/api/admin/news/delete/${id}`)
        .then((res) => {
          Swal.fire({
            icon: "success",
