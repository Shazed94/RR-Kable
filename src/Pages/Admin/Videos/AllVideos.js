...
const AllVideos = () => {
  ...
  const renderAllVideos = async () => {
    try {
      await axios.get(`${BACKEND_BASE_URL}/api/admin/videos`).then((res) => {
  ...
  const showSingleVideoData = (modalValue, id) => {
    axios.get(`${BACKEND_BASE_URL}/api/admin/videos/view/${id}`).then((res) => {
      setModalData(true);
  ...
  const deleteData = async (id) => {
    ...
    if (isConfirm) {
      axios
        .delete(`${BACKEND_BASE_URL}/api/admin/videos/delete/${id}`)
        .then((res) => {
          Swal.fire({
            icon: "success",
